...
- 通常のCIジョブでは全テスト(単体・統合・E2E・前提検証)を実行するが、前提検証テストのみ以下のように扱いを分ける。
  - **duckdb最新バージョンのジョブでAT-*が失敗した場合**: CI全体を失敗させるのではなく、6章の運用注記のとおり「design doc再検討が必要」というシグナルとして扱う。**採用済み**の実装はジョブ単位ではなく**ステップ単位**の `continue-on-error` である。具体的には、`test` ジョブ自体には `continue-on-error` を設定せず、ジョブ内の「前提検証テスト実行」ステップ(`assumptions` マーカー付きテストのみを実行)にのみ `continue-on-error: ${{ matrix.duckdb-version != '1.5.4' }}` を条件付きで設定する。これにより、同一ジョブ内の通常テスト(`-m "not assumptions"`、最低サポートバージョン・最新バージョンいずれの行でも)の失敗はジョブ全体を失敗させたまま維持しつつ、前提検証テストの失敗のみ最新duckdbの行に限って非ブロッキングにする。失敗の通知は週次スケジュール実行+Issue自動起票で行う(7.4節で確定)。
  - **最低サポートバージョンのジョブでAT-*が失敗した場合**: 通常の回帰として扱い、CIを失敗させる(サポート範囲内のバージョンでの前提崩れは看過しない)。

### 7.4 前提検証テスト失敗時の通知(2026-07-05確定)

最新duckdbバージョンでの前提検証テスト失敗の通知手段は、**週次スケジュール実行+GitHub Issue自動起票**で確定した(`.github/workflows/assumption-watch.yml`)。

- **週次スケジュール実行**: 毎週(月曜06:00 JST)、`assumptions` マーカーのテストをPyPI最新のduckdbに対して実行する。PRトリガーのCIと異なり、開発が止まっている期間でもduckdb新リリースによるドリフトを検知できる。`workflow_dispatch` による手動実行も可能。
- **Issue自動起票**: 失敗時は `assumption-drift` ラベル付きのGitHub Issueを自動起票する。同ラベルのopen Issueが既に存在する場合は新規起票せず、そのIssueへ実行結果をコメント追記する(週次実行による重複Issueの堆積を防ぐ)。Issue本文には「実装バグではなく設計前提の再検討シグナルである」旨(6章 運用注記)とpytest出力の末尾を含める。
- PRトリガーのCI(7.3節)側は従来どおり非ブロッキング警告のみとし、通知責務は本ワークフローに集約する。
- Slack/Discord等の外部サービス通知は採用しない(Webhook管理と外部依存が増えるため。GitHub通知経由で十分)。
...
- 前提検証テスト失敗時の通知の仕組みは確定済み(7.4節: 週次スケジュール実行+`assumption-drift` ラベル付きIssue自動起票)。
