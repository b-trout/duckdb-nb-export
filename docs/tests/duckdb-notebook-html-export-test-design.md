...
| CI | GitHub Actions(`.github/workflows/ci.yml`)。Linux(ubuntu-latest)・macOS(macos-latest)・Windows(windows-latest)。Windowsはベストエフォート対応(design doc 8章、ADR-008追記)のため、ロック依存の5テストのみ`skipif(sys.platform == "win32")`で除外し、他は3OSすべてで実行する |
...
- Linux/macOSは正式対応、Windowsはベストエフォート対応(design doc 8章、ADR-008追記)のため、CIマトリクスに `os: [ubuntu-latest, macos-latest, windows-latest]` を追加する(現状は `ubuntu-latest` 固定)。ロック保持中のOSファイルコピーに依存する5テストのみ `skipif(sys.platform == "win32")` でWindows実行から除外し、それ以外のテストはWindowsでも実行・合格を要求する。
...
| Windows環境でのロック保持中ui.dbコピーに依存するテスト(AT-004, AT-005, IT-001, IT-009, UT-R-001の5件) | 実機検証によりWindowsでは`PermissionError(13)`で恒久的に不可能と確定(design doc 6.3#11)。OSのファイルロック仕様によるものでコード側の回避策がないため、この5件のみ`skipif(sys.platform == "win32")`でWindows実行から除外する。それ以外のWindows環境でのテストはスコープ外ではなく、CIマトリクス(windows-latest)で通常どおり実行・合格を要求する | design doc 8章, ADR-008 |
...
- **実行環境再現テスト(UT-X-021)**: 保存形式v3にはATTACH・拡張ロード・シークレット・変数等の接続環境情報を保持するキーが存在しないことが判明した。ただし保存形式はDB名のみを保持するため、そこから導かれる適用セマンティクス(USE適用の対象・失敗時挙動)自体が未確定であり、環境再現の実装とテスト本体は保留中である(テストは明示的に `pytest.skip` する)。
