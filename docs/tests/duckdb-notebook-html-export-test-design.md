...
- 実際のDuckDB UI(`duckdb --ui`)で作成した `ui.db`(および `ui.db.wal`)を `tests/fixtures/ui_db/` 配下にバイナリのまま保持する。
...
- **再生成手順のスクリプト化**: フィクスチャは手作業では再現性がないため、`scripts/regenerate_ui_db_fixtures.py`(新規作成予定)のようなスクリプトで、実際に `duckdb --ui` を起動し、既定のnotebook操作(notebook作成・複数バージョン作成・複数セル追加等)を自動化して `ui.db` を生成し直す手順を用意する方針とする。スクリプトは手動実行(CI上で `duckdb --ui` を自動操作するのは非現実的なため)を前提とし、DuckDBバージョンアップ時やスキーマ変化検知時に開発者が明示的に再生成する運用とする。
...
- そのため、notebook JSONスキーマの実機調査(design doc 6.2#1、Phase 1実装の前提タスク)完了後に、テストコードから `notebook_versions` 相当のテーブル・レコードを直接組み立てる生成ヘルパー(例: `tests/helpers/synthetic_ui_db.py` の `build_ui_db(cells: list[CellSpec]) -> Path` のような関数)を用意する。
- このヘルパーは実DuckDB UIに依存せず、pytestのユニット/統合テストから任意のセル構成(SQL文字列・想定エラー種別等)を持つ `ui.db` を動的に生成できるようにする。
- **ブロッキング事項**: このヘルパーの実装はスキーマ調査(6.2#1)の結果に依存するため、調査完了までは合成フィクスチャを用いるテストは実装できない(8章のスコープ外・未決定事項に明記)。
...
- 正規化後の文字列を、`tests/golden/*.html` に保存済みの期待値と比較する。差分がある場合はテストが失敗し、意図した変更であれば golden ファイルを更新するワークフロー(例: `--snapshot-update` 相当のスクリプトまたはpytestオプション)を用意する。
...
| UT-X-021 | notebook JSONに接続環境情報(ATTACH・拡張ロード・シークレット・変数)が含まれる場合、可能な範囲で再現されてからセルが実行されること(6.2#1のスキーマ調査完了までブロック — 8.2節参照) | design doc 4.2節, ADR-008 |
...
| UT-RD-002 | セルSQLの文字列自体に `<`、`>`、`&` 等が含まれる場合もエスケープされて表示されること | design doc 5章, ADR-006 |
...
1. **DuckDB自体の挙動**(AT-001〜AT-008): design doc 6.3節「実機検証済み事項」の8項目を `scripts/verify_duckdb_assumptions.py` の内容から移植する(design doc 8章、9章)。`duckdb` パッケージ更新による前提変化を検知する。
...
### 6.1 DuckDB自体の挙動の監視(AT-001〜AT-008)
...
| AT-010 | notebook JSONのスキーマ | 実UI由来フィクスチャのnotebook JSONがPydanticモデルでパースできること(失敗=notebook JSONスキーマが変化したシグナル) | design doc 8章, 6.2#1, 4.1節 |

### 6.3 調査完了後に追加予定の前提検証テスト

以下は design doc 6.2の要調査事項が完了してから期待挙動を確定し、前提検証テストとして追加する予約枠である。

| ID(予約) | 検証項目 | 備考 | トレーサビリティ |
|---|---|---|---|
| AT-011 | 6.2#9: `connection.interrupt()` でセルを中断した場合にトランザクションがabort状態になるか | 調査完了後に期待挙動を確定し、AT-011として前提検証テストに追加する(abort状態になる場合はdesign doc 6.1#3の処理に合流する設計であるため、UT-X-012/013の期待値もこの結果に依存する — 8.3節参照) | design doc 6.2#9, 4.2節, ADR-007 |

### 運用注記

前提検証テスト(AT-001〜AT-010)が失敗した場合、それは**実装のバグを意味しない**。これらは「design doc・ADRが前提としている外部依存(DuckDB自体の挙動、DuckDB UIの非公式スキーマ)」を検証しているため、失敗は以下のいずれかを意味する。

- `duckdb` パッケージの新バージョンでdesign doc 6.3節の前提が変化した(例: 将来SAVEPOINTがサポートされる、abort挙動が変わる等)— AT-001〜AT-008。
- OS・実行環境の違いにより6.3節の前提が成立しない(特にAT-004・AT-005はロック機構のOS依存性が高い)— AT-001〜AT-008。
...
- `pytest.ini` または `pyproject.toml` の `[tool.pytest.ini_options]` に `markers = ["assumptions: 設計前提(DuckDB自体の挙動・DuckDB UIの非公式スキーマ)を検証する恒久テスト"]` を定義し、`@pytest.mark.assumptions` を付与する(監視対象2種の内訳は6章)。
- 通常のCIジョブでは全テスト(単体・統合・E2E・前提検証)を実行するが、前提検証テストのみ以下のように扱いを分ける。
  - **duckdb最新バージョンのジョブでAT-*が失敗した場合**: CI全体を失敗させるのではなく、6章の運用注記のとおり「design doc再検討が必要」というシグナルとして扱う。具体的には、当該ジョブを `continue-on-error: true` とするか、専用のSlack通知・Issue起票等の後続アクションにつなげ、mainブランチのマージ自体をブロックしない運用とすることを推奨する(最終的な運用ルールはCI導入時に確定する未決定事項として7.4に残す)。
...
### 8.2 6.2#1(notebook JSONスキーマ調査)完了までブロックされるテスト群

design doc 6.2#1は「notebook JSONの正確なスキーマ」の実機調査であり、Phase 1実装そのものの前提タスクとされている(design doc 6.1#12, 6.2#3, ADR-008)。テスト設計上、以下の項目はこの調査結果が確定するまで具体的な実装に着手できない。

- **2.2節の合成ui.dbフィクスチャ生成ヘルパー**: `notebook_versions` 相当のテーブルをテストコードから組み立てるには、正確なJSONスキーマ(セル構造・接続環境情報の格納形式等)が必要。
- **UT-X-014〜016(対象DB解決の優先順位)の一部**: notebook JSON内にどのような形式で接続情報が格納されるか(あるいは格納されないか)によって、「JSON内の情報から解決」経路の具体的なテストデータの組み方が変わる。優先順位のロジック自体(`--db` → JSON → `:memory:`)はモックデータでも検証可能だが、実データに近い形での再現テストは調査完了後に拡充する。
- **実行環境再現(ADR-008 決定2)に関する環境再現テスト(UT-X-021)**: ATTACH・拡張ロード・シークレット・変数等の再現範囲を検証するテストは、notebook JSON内にこれらの情報が実際にどう格納されるかが判明してから、具体的なフィクスチャとアサーションを確定する。

これらは本書の3〜5章のテストケース一覧には(調査完了後に実装可能なものとして)含めているが、実装着手順序としては6.2#1の調査完了を待つ必要がある点を明記する。

### 8.3 その他の未決定事項

- 7.3節で述べたとおり、前提検証テスト失敗時のCI通知の具体的な仕組みは未確定。
- ui.dbフィクスチャ(2.1節)の再生成スクリプト(`scripts/regenerate_ui_db_fixtures.py`)自体の実装は本書執筆時点では未着手であり、別途実装が必要。
- **UT-X-012/013の期待値は暫定である**: interrupt時のトランザクション状態遷移(design doc 6.2#9)は未調査であり、「interruptによる中断後にトランザクションがabort状態になるか(なる場合はabort系エラー処理に合流する — design doc 4.2節)」が確定していない。調査完了までは、UT-X-012(interrupt成功時に後続処理を継続)・UT-X-013(interrupt不能時の部分HTML+終了コード2)のアサーションのうち「中断後にabort処理へ合流するか否か」に関わる部分を暫定とし、調査結果の確定後にテスト期待値を確定するとともに、前提検証テストAT-011(6章の予約枠)を追加する。
